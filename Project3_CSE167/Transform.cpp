...
//    }
    ...
    while (it != children.end() && *it != nullptr) {
        Node* child = *it;
//        child->draw(shaderProgram, M_new);
        child->draw(shaderProgram, M_new/* *glm::scale(glm::mat4(1.0f), glm::vec3(0.9f))*/);
        it++;
    }
    
//    glm::vec3 origin = army->origin_in_CVV(army->origin); // x'
//    if (army->in_six_planes(origin) == 6) {
...
void Transform::update() {
    ...
    if (counter < 150) {
        if (LEFT) {
//            M = M * glm::rotate(glm::mat4(1.0f), -180 / 180.0f * glm::pi<float>(), glm::vec3(1.0f, 0.0f, 0.0f));
            walk(0.06f);
        }
        if (RIGHT) walk(-0.06f);
        counter++;
...
void Transform::drawSphere(GLuint shaderProgram, OBJObject * sphere) {
    for (list<Node*>::iterator it = children.begin(); it != children.end(); ++it) {
        Node* child = *it; //
...
// could make it return bool instead
//int Transform::in_six_planes(glm::vec3 x_prime) {
...
//        if (dist_to_plane(origin_NDC, normal) < radius_in_NDC(radius, normal)) count++;
//        if (check_origin_NDC(origin, normal) == true) count++;
...
//    if (dist_to_plane(x_prime, normal1) < radius) count++;
//    if (dist_to_plane(x_prime, normal2) < radius) count++;
//    if (dist_to_plane(x_prime, normal3) < radius) count++;
//    if (dist_to_plane(x_prime, normal4) < radius) count++;
//    if (dist_to_plane(x_prime, normal5) < radius) count++;
//    if (dist_to_plane(x_prime, normal6) < radius) count++;
    
//    if (dist_to_plane(origin_NDC, normal1) < radius_in_NDC(radius, normal1)) count++;
//    if (dist_to_plane(origin_NDC, normal2) < radius_in_NDC(radius, normal2)) count++;
//    if (dist_to_plane(origin_NDC, normal3) < radius_in_NDC(radius, normal3)) count++;
//    if (dist_to_plane(origin_NDC, normal4) < radius_in_NDC(radius, normal4)) count++;
//    if (dist_to_plane(origin_NDC, normal5) < radius_in_NDC(radius, normal5)) count++;
//    if (dist_to_plane(origin_NDC, normal6) < radius_in_NDC(radius, normal6)) count++;
